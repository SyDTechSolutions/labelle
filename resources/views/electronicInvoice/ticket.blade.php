  ...
  font-family: 'Times New Roman';
    ...
    @media print{
     ...
     <div class="ticket">
        ...
        <p>
          Cliente: {{ $invoice->emisor }}
          @if($invoice->identificacion_emisor)
            <br>Identificación: {{ $invoice->identificacion_emisor }}
          @endif
        </p>
        <table>
        <thead>
        ...
        <tbody>
            ...
            <tr class="summary">
                ...
                <td class="precio">{{ money($invoice->TotalVenta,'¢',2) }}</td>
            </tr>
            <tr class="summary">
                <td class="cantidad"></td>
                <td class="producto">Descuentos</td>
                <td class="precio">{{ money($invoice->TotalDescuentos,'¢',2) }}</td>
            </tr>
            <tr class="summary">
                <td class="cantidad"></td>
                <td class="producto">Impuestos</td>
                <td class="precio">{{ money($invoice->TotalImpuesto,'¢',2) }}</td>
            </tr>
            @if($invoice->MedioPago == '02')
            <tr class="summary">
                <td class="cantidad"></td>
                <td class="producto">IVA Devuelto</td>
                <td class="precio">(-) {{ money($invoice->TotalIVADevuelto, '¢', 2) }}</td>
            </tr>
            @endif
            <tr class="summary">
                <td class="cantidad"></td>
                <td class="producto">TOTAL</td>
                <td class="precio">{{ money($invoice->TotalComprobante,'¢',2) }}</td>
            </tr>
            <tr class="summary">
                <td class="cantidad"></td>
                <td class="producto">Pago con</td>
                <td class="precio">{{ money($invoice->pay_with,'¢',2) }}</td>
            </tr class="summary">
            <tr class="summary">
                <td class="cantidad"></td>
                <td class="producto">Cambio</td>
                <td class="precio">{{ money($invoice->change,'¢',2) }}</td>
            </tr>
        </tbody>
        </table>
...
<a class="oculto-impresion" href="/electronicinvoice">Regresar</a>
