    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ url('providers/'. $provider->id) }}" method="POST">
                        @csrf
 ...
 $(function () {
	...
	@if($provider)
	  	setTimeout(function(){
                ...
                $('#provincia option[value="{{ $provider->provincia }}"]').attr("selected", true);
                $('#provincia').change();
                $('#canton option[value="{{ $provider->canton }}"]').attr("selected", true);
			    $('#canton').change();
                $('#distrito option[value="{{ $provider->distrito }}"]').attr("selected", true);
                $('#distrito').change();
                $('#barrio option[value="{{ $provider->barrio }}"]').attr("selected", true);
            }, 100);
	@endif
